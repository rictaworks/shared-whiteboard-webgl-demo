...
フロントエンドは **Unity WebGL** と利用者から明示的に指定されたため、選定を行わない。指示のゲーム項に定める構成（Unity WebGL・C#・PlayerPrefs・シーン／UI／オブジェクトのコード生成・CLI ビルド・ページロード時の日付確認）を適用する。

配信先はゲーム項の既定である Unity Play ではなく、**自社の Cloudflare Workers Assets**（`shared-whiteboard-webgl-demo.rictaworks.jp`）とする。Unity Play は配布 zip 内の `index.html` を使わず独自プレイヤーが `Build` フォルダのファイルだけを読み込むため、`index.html` に置いた計測タグ・導線・API/WS の URL 注入がすべて無効になる。加えて参加 URL（`?b=<token>`）はページ URL から読む設計であるところ、Unity Play ではゲームがクロスオリジンの iframe で動くため原理的に到達せず、「URL を共有するだけで共同編集できる」という本デモの中核が成立しない。
...
| 層 | 技術 | デプロイ先 | 役割 |
|---|---|---|---|
| フロントエンド | Unity WebGL（C#） | Cloudflare Workers Assets（無料） | 描画・履歴・送信キュー・書き出し |
...
フロントエンドとサーバーは配信元が異なるため（フロントエンドは `shared-whiteboard-webgl-demo.rictaworks.jp`、中継・アプリケーション層は Railway の既定ドメイン）、Cookie によるセッション送信は成立しない。セッションキーは初回アクセス時にアプリケーション層が発行し、フロントエンドが PlayerPrefs に保持して、以後のすべての要求にヘッダとして付与する。セッションキーが DB レコードのオーナーキーである点はウェブと同一である。
...
- 中継・アプリケーション層は、配信元としてフロントエンドの配信元（`https://shared-whiteboard-webgl-demo.rictaworks.jp`）のみを受け入れること
...
| 配信 | フロントエンドは Cloudflare Workers Assets、中継・アプリケーション層は Railway。いずれもデスクトップからの手動デプロイ（`wrangler deploy` / `railway up`）とする |
| 測定 | 行わない |
| 保守・監視 | 行わない |
