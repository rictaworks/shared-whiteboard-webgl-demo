...
namespace Whiteboard.Boot
{
    ...
    public class Boot : MonoBehaviour
    {
        ...
        {
            // 実機バグ修正（2026-09-21・Issue #37）：InputBridgeはキャンバス上の
            // Pointerイベントを画面状態に関わらず無条件にバッファへ蓄積し続けるため、
            // 遷移前の画面で押したボタンのdown/upが未処理のまま残り、遷移後の最初の
            // フレームで新しい画面上の入力として誤って処理されていた（遷移直前のボタン
            // 座標に意図しない点が描かれる）。切り替えの直前にバッファを読み捨てる。
            InputBridge.Flush();
